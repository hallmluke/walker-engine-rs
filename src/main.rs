...
impl WalkerEngine {
    pub fn new(event_loop: &winit::event_loop::EventLoop<()>) -> WalkerEngine {
        let window =
        ...
        let msaa_samples = WalkerEngine::get_max_usable_sample_count(&instance, physical_device);
        ...
        let render_pass = WalkerEngine::create_render_pass(
            &instance,
            &device,
            physical_device,
            swapchain_stuff.swapchain_format,
        ...
        let (graphics_pipeline, pipeline_layout) = WalkerEngine::create_graphics_pipeline(
            &device,
            render_pass,
        ...
        let (color_image, color_image_view, color_image_memory) =
            WalkerEngine::create_color_resources(
                &device,
                swapchain_stuff.swapchain_format,
                swapchain_stuff.swapchain_extent,
        ...
        let swapchain_framebuffers = WalkerEngine::create_framebuffers(
            &device,
            render_pass,
        ...
        let (texture_image, texture_image_memory, mip_levels) = WalkerEngine::create_texture_image(
            &device,
            command_pool,
            graphics_queue,
            &physical_device_memory_properties,
        ...
        let texture_sampler = WalkerEngine::create_texture_sampler(&device, mip_levels);
        ...
        let command_buffers = WalkerEngine::create_command_buffers(
            &device,
            command_pool,
            graphics_pipeline,
...
impl WalkerEngine {
    ...
    fn create_graphics_pipeline(
        ...
        let vert_shader_module = share::create_shader_module(
            device,
            include_bytes!("../shaders/spv/shader-depth-vert.spv").to_vec(),
        );
        let frag_shader_module = share::create_shader_module(
            device,
            include_bytes!("../shaders/spv/shader-depth-frag.spv").to_vec(),
        );
...
impl VulkanApp for WalkerEngine {
    ...
    fn recreate_swapchain(&mut self) {
        ...
        self.render_pass = WalkerEngine::create_render_pass(
            &self.instance,
            &self.device,
            self.physical_device,
            self.swapchain_format,
        ...
        let color_resources = WalkerEngine::create_color_resources(
            &self.device,
            self.swapchain_format,
        ...
        self.swapchain_framebuffers = WalkerEngine::create_framebuffers(
            &self.device,
            self.render_pass,
        ...
        self.command_buffers = WalkerEngine::create_command_buffers(
            &self.device,
            self.command_pool,
...
fn main() {
    ...
    let vulkan_app = WalkerEngine::new(&program_proc.event_loop);
