...
impl WalkerEngine {
    pub fn new(window: &winit::window::Window) -> WalkerEngine {
        ...
        let surface_stuff = WalkerEngine::create_surface(&entry, &instance, &window);
        let (debug_utils_loader, debug_merssager) =
            utility::debug::setup_debug_utils(VALIDATION.is_enable, &entry, &instance);
        let physical_device = WalkerEngine::pick_physical_device(&instance, &surface_stuff);
        let (device, family_indices) = WalkerEngine::create_logical_device(
            &instance,
            physical_device,
            &VALIDATION,
            &surface_stuff,
        ...
        let swapchain_stuff = WalkerEngine::create_swapchain(
            &instance,
            &device,
            physical_device,
            &surface_stuff,
    ...
    fn pick_physical_device(
        ...
        let result = physical_devices.iter().find(|physical_device| {
            WalkerEngine::is_physical_device_suitable(
                instance,
                **physical_device,
                surface_stuff,
            )
    ...
    fn is_physical_device_suitable(
        ...
        let indices = WalkerEngine::find_queue_family(instance, physical_device, surface_stuff);

        let is_queue_family_supported = indices.is_complete();
        let is_device_extension_supported =
            WalkerEngine::check_device_extension_support(instance, physical_device);
        let is_swapchain_supported = if is_device_extension_supported {
            let swapchain_support = WalkerEngine::query_swapchain_support(physical_device, surface_stuff);
    ...
    fn create_swapchain(
        ...
        let surface_format = WalkerEngine::choose_swapchain_format(&swapchain_support.formats);
        let present_mode =
            WalkerEngine::choose_swapchain_present_mode(&swapchain_support.present_modes);
        let extent = WalkerEngine::choose_swapchain_extent(&swapchain_support.capabilities);
...
fn main() {
    ...
    let walker_app = WalkerEngine::new(&window);
